...
> Актуальное состояние на 2026-07-19. Документ перезаписывается
...
ui/
  main.slint     разметка главного окна (меню, тулбар, вкладки, редактор,
                 строка состояния) и свои компоненты (Editor, CloseButton)
...
Редактор — свой компонент `Editor` (низкоуровневый `TextInput` внутри
`ScrollView`, по образцу внутреннего `TextEditBase` Slint), а не `TextEdit`
из std-widgets: тому нечем отдать позицию курсора, а строке состояния она
нужна. Используется недокументированное свойство `TextInput.
cursor-position-byte-offset` — при обновлении Slint проверять changelog.
Прокрутка вслед за курсором и контекстное меню — обязанность компонента
(см. dev-log, запись 16).

Панель инструментов: Новый/Открыть/Сохранить дёргают те же callbacks, что и меню;
Вырезать/Копировать/Вставить вызывают функции `Editor` прямо в разметке
...
callback text-edited(string)   →    выставить dirty (текст — лениво, sync_active_doc)
                                    + пересчитать «Символов: N»
callback cursor-moved(int)     →    пересчитать «Стр N, стлб M» (caret_position)
...
Строка состояния — три текстовых свойства (`status-caret`, `status-size`,
`status-encoding`), готовые строки для них собирает Rust. Обновления:
движение курсора (change callback на `cursor-position-byte-offset` →
`cursor-moved`) — позиция; ввод текста и `apply_replacement` — размер;
`show_doc` (смена вкладки, открытие файла, старт) — всё разом, включая
кодировку. Позиция считается `caret_position` из байтового смещения:
строка — по `\n`, столбец — в символах, сырое смещение прижимается
к границе символа.
